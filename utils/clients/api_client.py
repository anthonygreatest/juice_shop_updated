...
class APIClient:
    ...
    def register_user(self, data, headers=None):
        return self.session.post(
            url=f'{self.endpoints.register}',
            data=data,
            headers=headers
        )

    def login_user(self, data, headers=None):
        return self.session.post(
            url=f'{self.endpoints.login}',
            data=data,
            headers=headers
        )

    def set_security_answer(self, data):
        return self.session.post(
            url=f'{self.endpoints.security_answer}',
            json=data
        )

    def change_password(self, data, headers=None):
        return self.session.post(
            url=f'{self.endpoints.change_password}',
            json=data,
            headers=headers
        )
    ...
    def add_to_basket(self, data, headers=None):
        resp = self.session.post(
            url=f'{self.endpoints.add_to_basket}',
            json=data,
            headers=headers
        )
        return resp, AddToBasketRespSchema(**resp.json())

    def logout(self, headers=None):
        resp = self.session.get(
            url=f'{self.endpoints.logout}',
            headers=headers
        )
        return resp, LogoutRespSchema(**resp.json())

    def add_or_remove_one_item(self, order_id, data, headers=None):
    ...
    def delete_product_from_basket(self, order_id, headers=None):
        return self.session.delete(
            url=f'{self.endpoints.add_to_basket}/{int(order_id)}',
            headers=headers
        )

    def create_address(self, data, headers=None):
        return self.session.post(
            url=f'{self.endpoints.add_address}',
            json=data,
            headers=headers
        )
    ...
    def add_credit_card(self, data, headers=None):
        ...
        resp = self.session.post(
            url=f'{self.endpoints.add_credit_card}',
            json=data,
            headers=headers
        )
        return resp, AddCreditCardRespSchema(**resp.json())

    def checkout(self, data, basket_id, headers=None):
        return self.session.post(
            url=f'{self.endpoints.checkout(basket_id)}',
            json=data.model_dump(by_alias=True, mode='json'),
            headers=headers
    ...
    def get_order_history(self, headers=None):
        ...
        return self.session.get(
            url=f'{self.endpoints.order_history}',
            headers=headers
        )
    ...
    def get_credit_card(self, headers=None):
        resp = self.session.get(
            url=f'{self.endpoints.add_credit_card}',
            headers=headers
        )
        return resp, ExistingCreditCardSchema(**resp.json())

    def delete_card(self, card_id, headers=None):
        return self.session.delete(
            url=f'{self.endpoints.add_credit_card}/{card_id}',
            headers=headers
        )

    def recycle(self, data, headers=None):
        resp = self.session.post(
            url=f'{self.endpoints.recycle}',
            json=data,
            headers=headers
        )
        return resp, RecycleRespSchema(**resp.json())
    ...
    def request_data_erasure(self, data, headers=None):
        return self.session.post(
            url=f'{self.endpoints.data_erasure}',
            data=data,
            headers=headers
        )
    ...
    def become_deluxe_member(self, data, headers=None):
        return self.session.post(
            url=f'{self.endpoints.deluxe_membership}',
            json=data,
            headers=headers
        )
    ...
    def file_a_complaint(self, data, headers=None):
        resp = self.session.post(
            url=f'{self.endpoints.complaint}',
            json=data,
            headers=headers
        )
        return resp, SendComplaintRespSchema(**resp.json())

    def upload_photo(self, files, headers=None):

        resp = self.session.post(
            url=f'{self.endpoints.photowall}',
            headers=headers,
            files=files
        )
        return resp, UploadPhotoRespSchema(**resp.json())
    ...
    def get_photos(self, headers=None):
        ...
        return resp, SavedPicsRespSchema(**resp.json())

    def ask_robot(self, data, headers=None):
        return self.session.post(
            url=f'{self.endpoints.robot}',
            headers=headers,
            json=data
        )

    def set_profile_name(self, data, headers=None):
        return self.session.post(
            url=f'{self.endpoints.profile_name}',
            data=data,
            headers=headers,
    ...
    def request_data_export(self, data, headers=None):
        resp = self.session.post(
            url=f'{self.endpoints.data_export}',
            json=data,
            headers=headers
        )
        user_data = json.loads(resp.json()['userData'])
        final_data = resp.json()
        final_data['userData'] = user_data
        return final_data, DataExportRespSchema(**final_data), resp

    def request_captcha(self, headers=None):
        return self.session.get(
            url=f'{self.endpoints.data_export_captcha}',
            headers=headers
        )

    def get_product_in_basket(self, headers):
        return self.session.get(
            url=f'{self.endpoints.basket}',
            headers=headers
        )

    def get_reviews(self, product_id, headers):
        resp = self.session.get(
            url=f'{self.endpoints.all_reviews(product_id)}',
            headers=headers
        )
        return resp, GetReviewsRespSchema(**resp.json())
    ...
    def like_a_review(self, data, headers):
        resp = self.session.post(
            url=f'{self.endpoints.reviews}',
            json=data,
            headers=headers
        )
        return resp, LikeReviewRespSchema(**resp.json())
